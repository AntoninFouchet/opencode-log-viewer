...
let REFRESH_INTERVAL = parseInt(localStorage.getItem('refresh_interval') || DEFAULT_REFRESH_INTERVAL);
...
async function init() {
    ...
    if (isConnected) {
        ...
        showError('Impossible de se connecter au serveur OpenCode. Vérifiez que le serveur est démarré sur ' + API_URL);
        return;
    }
...
function displaySession(session, messages) {
    // Header
    document.getElementById('session-title').textContent = session.title || 'Sans titre';
    document.getElementById('session-status').textContent = session.status || 'unknown';
    document.getElementById('session-status').className = `status status-${session.status || 'unknown'}`;
    document.getElementById('session-date').textContent = formatDateTime(session.time?.created);
...
function handleSessionEvent(event) {
    ...
    switch (event.type) {
        ...
        case 'session.status':
            ...
            document.getElementById('session-status').className = `status status-${status}`;
            break;
...
function renderStats(session, messages) {
    ...
    statsContent.innerHTML = `
        ...
        ${Object.entries(partTypes).map(([type, count]) => `
            <div class="stat">
...
function renderTools(messages) {
    ...
    messages.forEach(msg => {
        (msg.parts || []).forEach(part => {
            if (part.type === 'tool' && part.tool?.name) {
    ...
    if (toolEntries.length === 0) {
        toolsContent.innerHTML = '<div class="empty-stats">Aucun outil utilisé</div>';
        return;
    }
    ...
    toolsContent.innerHTML = toolEntries.map(([name, count]) => `
        <div class="tool-item">
...
function setupEventListeners() {
    ...
    statusFilter.addEventListener('change', (e) => {
        filterSessions(searchInput.value, e.target.value);
    });
...
function filterSessions(searchTerm = '', status = '') {
...
function saveSettings() {
    ...
    console.log(`✅ Paramètres sauvegardés: URL=${API_URL}, Intervalle=${REFRESH_INTERVAL}ms`);
}
